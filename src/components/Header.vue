<template>
  <nav class="navbar navbar-light bg-light">
    <h1 class="navbar-brand mb-0">
      <span id="header-start">CRISIS</span>
      <span id="header-end">app</span>
    </h1>
    <p class="font-smaller">
      In a natural disaster emergency, we help you find the help you need, or find where and how you can help.
    </p>
    
    <div class="alert alert-primary w-100" role="alert">
        This website is still under active development, go-live should only be a few hours away &lt;3
    </div>
    
    <div style="text-align:center" class="alert alert-primary w-100" role="alert">
     Disclaimer. This is a community-sourced directory moderated by volunteers.  We do our best to make sure information is accurate.  Information may change or become out-of-date. Please verify.
    </div>
    
</nav>
</template>

<style scoped>
nav h1 {
  font-size: xx-large;
  font-weight: bold;
}

#header-start {
  color: #02909e;
}
#header-end {
  color: #384b71;
}
</style>
